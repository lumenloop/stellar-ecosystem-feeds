<?php
class CssLostDateBridge extends BridgeAbstract
{
    const MAINTAINER = 'Raph';
    const NAME = 'CSS Lost Date Bridge';
    const URI = 'https://github.com/lumenloop/stellar-ecosystem-feeds/rss-bridges/';
    const DESCRIPTION = 'Convert any site to RSS feed using CSS selectors and specify date from array of dates elements (Advanced Users)';
    const PARAMETERS = [
        [
            'home_page' => [
            ...
            'date_selector_index' => [
                ...
                'title' => 'Index of the date element to use if multiple elements match the date selector. Defaults to 0 (first element).',
                'exampleValue' => '0'
            ],
            'author_selector' => [
        ...
        foreach ($elements as $element) {
            ...
            if (!empty($date_selector)) {
                ...
                if (!empty($dateElements)) {
                    $index = $this->getInput('date_selector_index');
                    if (!is_numeric($index) || $index < 0) {
                        $index = 0;
                    }
                    $dateElement = isset($dateElements[$index]) ? $dateElements[$index] : $dateElements[0];
    ...
    protected function expandEntryWithSelector(
        ...
        $date_selector = $this->getInput('date_selector');
        $date_format   = $this->getInput('date_format');
        $date_selector_index = $this->getInput('date_selector_index');
        if (!empty($date_selector)) {
            $dateElements = $entry_html->find($date_selector);
            if (!empty($dateElements)) {
                $index = (!is_numeric($date_selector_index) || $date_selector_index < 0) ? 0 : $date_selector_index;
                $dateElement = isset($dateElements[$index]) ? $dateElements[$index] : $dateElements[0];
                $dateString = trim($dateElement->plaintext);
                if (!empty($date_format)) {
                    $dt = DateTime::createFromFormat($date_format, $dateString);
